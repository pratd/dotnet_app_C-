...
namespace Library
{
    public partial class adminBookInventory : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Book Already exist, try another book ID');</script>");
            }
            else
            {
                addNewBook();
        ...
        void addNewBook()
        {
            try
            {
                ...
                string filepath = "~/bppk_inventory/books1.png";
                string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
                FileUpload1.SaveAs(Server.MapPath("book_inventory/" + filename)); // to save the file
                filepath = "~/book_inventory/" + filename;

                SqlCommand cmd = new SqlCommand("INSERT INTO book_master_tbl(book_id,book_name," +
                    "genre,author_name,publisher_name,publish_date,language,edition,book_cost," +
            ...
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            Response.Write("<script>alert('Actual Stock value " +
                                "cannot be less than the Issued books');</script>");
                        }
                        else
                        {
                    ...
                    genres = genres.Remove(genres.Length - 1); //remove the last comma
